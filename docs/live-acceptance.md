...
| Real OAuth | PASS | [Connection evidence](../evidence/google-connection.json): app shows Connected after scope and Gmail profile validation; denial/reconnect reported by user, disconnect pending |
| Real Google free/busy | BLOCKED | Account connected and owned calendar configured; awaiting labeled test request and live availability read |
...
After a new explicit user instruction, one incoming test request was sent between the two user-controlled accounts and placed under the test label. InboxOps has not yet created a real draft, sent a reply, or created a Calendar event. Passing local inference, retrieval, or offline CI does not complete this live acceptance procedure. At the user's request, a dedicated Cloud project was created, Gmail and Calendar APIs enabled, External/Testing audience configured, and the four documented scopes saved. The Web client has only the exact loopback callback. Its downloaded JSON is outside Git and restricted to the current Windows user; local configuration is ignored. The user explicitly selected an existing receiving account with an isolated test label after being informed that OAuth read access covers more than one label. An empty private owned test calendar was created. InboxOps now runs in live mode on loopback port 4317, which was free; no other project was stopped. The user completed Google consent and the live app displayed the configured account as Connected. The user reports a cancellation followed by successful reconnection; the denial was not independently captured. Disconnect remains untested. See [connection evidence](../evidence/google-connection.json).
